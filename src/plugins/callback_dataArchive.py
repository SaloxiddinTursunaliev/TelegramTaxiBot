...
def callback_inline(call):
  ...
  if call.message:
    ...
    if call.data == "main":
      ...
      markup = types.InlineKeyboardMarkup()
      markup.row_width = 1
      markupi00 = types.InlineKeyboardButton("Taxi", callback_data='Taxi')
      markupiaa = types.InlineKeyboardButton("Labo", callback_data='Labo')
      markupibb = types.InlineKeyboardButton("Motoroller", callback_data='Motoroller')
      markupicc = types.InlineKeyboardButton("Munis", callback_data='Munis')
      markupidd = types.InlineKeyboardButton("Kamaz", callback_data='Kamaz')
      markupiee = types.InlineKeyboardButton("Zil", callback_data='Zil')
      markupiff = types.InlineKeyboardButton("Traktor", callback_data='Traktor')
      markupigg = types.InlineKeyboardButton("Moskvich", callback_data='Moskvich')
      markupihh = types.InlineKeyboardButton("Jiguli", callback_data='Jiguli')
      markup.add(markupi00, markupiaa, markupibb, markupicc, markupidd, markupiee, markupiff, markupigg, markupihh)
      markup.row_width = 3
      markupii = types.InlineKeyboardButton("⚙️", callback_data='settingsmain')
    ...
    if call.data == "settingslang":
      msgid = call.inline_message_id
      markup = types.InlineKeyboardMarkup()
      markupib = types.InlineKeyboardButton("🇺🇿 O'zbekcha", callback_data='settingslangen')
    ...
    for rowsend2 in recordsend2:
        if call.data == rowsend2[5]:
            ...
            def step4(call):
                ...
                elif rowsend2userid == str(call.from_user.id):
...
#user side
    cur.execute('SELECT * from addsub where userid0 = %s', ["str(call.from_user.id)"])
    record44 = cur.fetchall()
    for row44 in record44:
        if call.data == row44[1]:
            addsubname=row44[0]
            ...
            cur.execute('SELECT * from bargain where ishchi_id = %s and user_id=%s and orde=%s', [addsubishchiid, str(call.from_user.id), addsuborde])
            record0 = cur.fetchall()
            for row0 in record0:
                row01 = row0[0]
                row02 = row0[1]
            ...
            cur.execute('UPDATE bargain SET narxi=%s, send2orde=%s WHERE ishchi_id=%s and orde=%s', [y, send2, addsubishchiid, addsuborde])
            db.commit()
            
            cur.execute('SELECT address, vazifa from buyurtma where id=%s and orde=%s', [row01, addsuborde])
            record = cur.fetchall()
            for row in record:
                useraddress = row[0]
                uservazifa = row[1]
              
            xabar = "Manzil: " + useraddress + "\nVazifa: " + uservazifa
            
            ikkip = "+2000" + addsubuserid + orde2
            beshp = "+5000" + addsubuserid + orde2
            onp = "+10000" + addsubuserid + orde2
            ikkim = "-2000" + addsubuserid + orde2
            beshm = "-5000" + addsubuserid + orde2
            onm = "-10000" + addsubuserid + orde2
            ...
            markup = types.InlineKeyboardMarkup()
            markup.row_width = 1
            markupib = types.InlineKeyboardButton(y, callback_data='narxni_kiritib_korish1')
            markup.add(markupib)
            markup.row_width = 3
            markupia = types.InlineKeyboardButton("+ 2000 so'm", callback_data = ikkip)
            markupib = types.InlineKeyboardButton("+ 5000 so'm", callback_data = beshp)
            markupic = types.InlineKeyboardButton("+ 10000 so'm", callback_data = onp)
            markupid = types.InlineKeyboardButton("- 2000 so'm", callback_data = ikkim)
            markupie = types.InlineKeyboardButton("- 5000 so'm", callback_data = beshm)
            markupif = types.InlineKeyboardButton("- 10000 so'm", callback_data = onm)
            markup.add(markupia, markupib, markupic, markupid, markupie, markupif)
            markup.row_width = 1
            markupib = types.InlineKeyboardButton(send_button, callback_data = send2)
            markup.add(markupib)
    ...
    if call.data == "rasm":
      msgid = call.inline_message_id
      markup = types.InlineKeyboardMarkup()
      markupib = types.InlineKeyboardButton("🇱🇷 Meat", callback_data='rasm1')
      markup.add(markupib)
      bot.edit_message_text(inline_message_id=msgid, chat_id=call.message.chat.id, message_id=call.message.message_id, text="Please choose products!. \nلطفا یک زبان انتخاب کنید. \n", parse_mode="Markdown", reply_markup=markup)
      markup = types.InlineKeyboardMarkup()
      markupib = types.InlineKeyboardButton("🇱🇷 Milk", callback_data='rasm1')
      markup.add(markupib)
      bot.edit_message_text(inline_message_id=msgid, chat_id=call.message.chat.id, message_id=call.message.message_id, text="Please choose products!. \nلطفا یک زبان انتخاب کنید. \n", parse_mode="Markdown", reply_markup=markup)
      markup = types.InlineKeyboardMarkup()
      markupib = types.InlineKeyboardButton("🇱🇷 Water", callback_data='rasm1')
      markup.add(markupib)
      bot.edit_message_text(inline_message_id=msgid, chat_id=call.message.chat.id, message_id=call.message.message_id, text="Please choose products!. \nلطفا یک زبان انتخاب کنید. \n", parse_mode="Markdown", reply_markup=markup)
      markup = types.InlineKeyboardMarkup()
      markupib = types.InlineKeyboardButton("🇱🇷 Sweets", callback_data='rasm1')
      markup.add(markupib)
      bot.edit_message_text(inline_message_id=msgid, chat_id=call.message.chat.id, message_id=call.message.message_id, text="Please choose products!. \nلطفا یک زبان انتخاب کنید. \n", parse_mode="Markdown", reply_markup=markup)
    
    if call.data == "rasm1":
      msgid = call.inline_message_id
      # bot.send_message(call.message.chat.id, "asas")
      
      def sss(call):
          markup = types.InlineKeyboardMarkup()
          markup.row_width = 1
          markupib = types.InlineKeyboardButton("🥩 Sabzavot va mevalar", callback_data='rasm21')
          markupic = types.InlineKeyboardButton("🥖 Go'sht mahulotlar", callback_data='rasm21')
          markupid = types.InlineKeyboardButton("🍬 Sut mahsulotlar", callback_data='rasm21')
          markupie = types.InlineKeyboardButton("🍏 Yuvish vositalar", callback_data='rasm21')
          markupif = types.InlineKeyboardButton("🍶 Ichimliklar", callback_data='rasm21')
          markupig = types.InlineKeyboardButton("🧂 Shirinliklar", callback_data='rasm21')
          markupic1 = types.InlineKeyboardButton("🥖 Non mahsulotlar", callback_data='rasm21')
          markupid1 = types.InlineKeyboardButton("🍬 Boqqolik mahsulotlar", callback_data='rasm21')
          markupie1 = types.InlineKeyboardButton("🍏 Konstovar", callback_data='rasm2')
          markup.add(markupib, markupic, markupid, markupie, markupif, markupig, markupic1, markupid1, markupie1)
          return markup
          
      bot.edit_message_text(inline_message_id=msgid, chat_id=call.message.chat.id, message_id=call.message.message_id, text="Please choose:", parse_mode="Markdown", reply_markup=sss(call))
    ...
    if call.data == "rasm21":
      ...
      def sss1(call):
          markup = types.InlineKeyboardMarkup()
          markup.row_width = 1
          markupib = types.InlineKeyboardButton("🥩 SABZAVOTLAR VA MEVALAR", callback_data='rasm2')
          markupic = types.InlineKeyboardButton("🍗🍖 Go'sht mahulotlar", callback_data='rasm2')
          markupid = types.InlineKeyboardButton("🍬 Sut mahsulotlar", callback_data='rasm2')
          markupie = types.InlineKeyboardButton("🍏 Yuvish vositalar", callback_data='rasm2')
          markupif = types.InlineKeyboardButton("🍶 Ichimliklar", callback_data='rasm2')
          markupig = types.InlineKeyboardButton("🍬🍫🍦🍩 Shirinliklar", callback_data='rasm2')
          markupic1 = types.InlineKeyboardButton("🥖 Non mahsulotlar", callback_data='rasm2')
          markupid1 = types.InlineKeyboardButton("🍬 Boqqolik mahsulotlar", callback_data='rasm2')
          markupie1 = types.InlineKeyboardButton("🍏 Konstovar", callback_data='rasm2')
          markupi2b = types.InlineKeyboardButton("⬅️ Asosiy menu", callback_data='rasm1')
          markupi2d = types.InlineKeyboardButton("⬅️ Orqaga", callback_data='rasm1')
          markupi2e = types.InlineKeyboardButton("🧺 Savatcha", callback_data='rasm2')
          markupi3b = types.InlineKeyboardButton("⚙️ Settings", callback_data='rasm1')
          markupi3d = types.InlineKeyboardButton("🏤 Home", callback_data='rasm1')
          markupi3e = types.InlineKeyboardButton("🏫 Do'konlar", callback_data='rasm1')
          markup.add(markupib, markupic, markupid, markupie, markupif, markupig, markupic1, markupid1, markupie1)
          markup.row_width = 3
          markup.add(markupi2b, markupi2d, markupi2e, markupi3b, markupi3d, markupi3e)
          return markup
          
      bot.edit_message_text(inline_message_id=msgid, chat_id=call.message.chat.id, message_id=call.message.message_id, text="Please choose:", parse_mode="Markdown", reply_markup=sss1(call))
    ...
    if call.data == "ishchi-labo2":
      ...
      bot.edit_message_text(inline_message_id=msgid, chat_id=call.message.chat.id, message_id=call.message.message_id, text="Please choose:", parse_mode="Markdown", reply_markup=sss111(call))
    ...
    if call.data == "rasm2":
      ...
      bot.edit_message_text(inline_message_id=msgid, chat_id=call.message.chat.id, message_id=call.message.message_id, text="Please choose productsaa. \nلطفا یک زبان انتخاب کنید. \n", parse_mode="Markdown", reply_markup=pho(call))
